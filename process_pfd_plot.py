...
def fold_snr_plot(merged_df, src_name, band, output_dir):
    ...
    src_ra = merged_df[merged_df['SNR'] == 1]['RA']
    src_dec = merged_df[merged_df['SNR'] == 1]['DEC']
...
def sim_snr_plot(sim_file, src_name, band, output_dir):
    '''
    ...
    output_path = os.path.join(output_dir, f"SNRMap_{src_name}_B{band}.png")
    plt.savefig(output_path)
    plt.show()

    return sim_sm

def residual_plot(fold_sm, sim_sm, src_name, band):
    '''
    ...
    plt.gca().xaxis.set_major_formatter(FormatStrFormatter('%.4f'))
    plt.legend()
    # plt.savefig(output_dir / f"SNRMapRES_{src_name}_B{band}.png")
    plt.show()
...
def main():
    try:
        ...
        print('done ahdr data')
        new_data = extract_snr(pfd_dir_path,ahdr_data,nbeams, pfd_code)
        print('done new data')
        fold_sm = fold_snr_plot(new_data, src_name, band, output_dir_path)
        print('done fold sm')
        sim_sm = sim_snr_plot(sim_file_path, src_name, band, output_dir_path)
        print('done sim sm')
        residual_plot(fold_sm, sim_sm, src_name, band)

        print(f"SNR Map data and plot saved to {output_dir_path}")
