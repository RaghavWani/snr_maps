...
def snr_plot(header_df, df, new_df, dm_tol, toA, dm_ver, source_ra, source_dec, output_dir, mjd, cand_ra_dec):
    ...
    ax.legend(loc="top")
    ax.format(xlim=(ra_min, ra_max), ylim=(dec_min, dec_max), suptitle=f"Single Pulse SNR Map (Post-Classify), T={toA}")   
...
def spatial_snr_plot(header_df, df, new_df, dm_tol, toA, dm_ver, source_ra, source_dec, output_dir, mjd, cand_ra_dec):
    ...
    np.save(os.path.join(output_dir, "SNR_postclassify_skymap.npy"), snr_image)
    ...
    ax.legend(loc="top")
    ax.format(xlim=(ra_min, ra_max), ylim=(dec_min, dec_max), suptitle=f"Single Pulse Sky-SNR Map (Post-Classify), T={toA}")
    ax.set_xlabel('Right Ascension')
    ax.set_ylabel('Declination')
    ax.set_aspect('auto')
